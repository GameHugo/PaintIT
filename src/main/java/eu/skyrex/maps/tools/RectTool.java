...
public class RectTool implements PaintTool {
    ...
    public void tick(CanvasManager canvasManager, Pixel pixel) {
        ...
        graphics.fillRect(Math.min(start.x(), pixel.x()), Math.min(start.y(), pixel.y()), diff(start.x(), pixel.x()), diff(start.y(), pixel.y()));
